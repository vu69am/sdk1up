...
static long ioctl_ioctl(struct file *file_p, unsigned int cmd,
			unsigned long arg)
{
	char data[1024];
	int ret = 0;
...
#ifdef CONFIG_COMPAT
static long compat_ioctlchr_ioctl(struct file *filp, unsigned int cmd,
				  unsigned long arg)
{
...
static int __init init_extenal_ioctl(void)
{
	...
	dev = device_create(ioctl_char_class, NULL, ioctl_devid, NULL,
			    IOCTL_CHAR_DEVICE_NAME);
	if (IS_ERR(dev)) {
