...
static struct aicwf_usb_buf *
aicwf_usb_msg_rx_buf_get(struct aic_usb_dev *usb_dev)
{
	unsigned long flags;
	struct aicwf_usb_buf *usb_buf;
	...
	if (list_empty(&usb_dev->msg_rx_free_list)) {
		...
		usb_buf = list_first_entry(&usb_dev->msg_rx_free_list,
					   struct aicwf_usb_buf, list);
		list_del_init(&usb_buf->list);
	}
...
static void aicwf_usb_msg_rx_buf_put(struct aic_usb_dev *usb_dev,
				     struct aicwf_usb_buf *usb_buf)
{
	unsigned long flags;
...
static void usb_txc_sta_flowctrl(struct aicwf_usb_buf *usb_buf,
				 struct aic_usb_dev *usb_dev)
{
	...
	if (sta_idx < NX_REMOTE_STA_MAX &&
	    !(hostdesc->host.flags & TXU_CNTRL_MGMT)) {
		...
		atomic_dec(
			&usb_dev->rwnx_hw->sta_flowctrl[sta_idx].tx_pending_cnt);
		//printk("sta:%d, pending:%d, flowctrl=%d\n", sta->sta_idx, sta->tx_pending_cnt, sta->flowctrl);
		if (RWNX_VIF_TYPE(vif) == NL80211_IFTYPE_AP) {
			if (atomic_read(&usb_dev->rwnx_hw->sta_flowctrl[sta_idx]
						 .tx_pending_cnt) <
				    AICWF_USB_FC_PERSTA_LOW_WATER &&
				...
				if (!usb_dev->tbusy)
					rwnx_wake_sta_all_queues(
						sta, usb_dev->rwnx_hw);
				usb_dev->rwnx_hw->sta_flowctrl[sta_idx]
					.flowctrl = 0;
			}
		}
...
static void aicwf_usb_rx_complete(struct urb *urb)
{
		...
		if (urb->status < 0) {
			...
			if (g_rwnx_plat->wait_disconnect_cb == false) {
				...
				if (atomic_read(&aicwf_deinit_atomic) > 0) {
					...
					AICWFDBG(
						LOGINFO,
						"%s need to wait for disconnect callback \r\n",
						__func__);
				} else {
	...
	if (usb_dev->state == USB_UP_ST) {
		...
		if (!aicwf_rxbuff_enqueue(usb_dev->dev, &rx_priv->rxq,
					  rx_buff)) {
			spin_unlock_irqrestore(&rx_priv->rxqlock, flags);
			usb_err("rx_priv->rxq is over flow!!!\n");
			aicwf_prealloc_rxbuff_free(rx_buff,
						   &rx_priv->rxbuff_lock);
			aicwf_usb_rx_buf_put(usb_dev, usb_buf);
			aicwf_usb_rx_submit_all_urb_(usb_dev);
			return;
	...
	if (usb_dev->state == USB_UP_ST) {
		...
		if (aicwf_usb_rx_aggr) {
			...
			if (rx_priv->rx_reassemble_skb) {
				...
				struct sk_buff *reassemble_skb =
					rx_priv->rx_reassemble_skb;
				bool reassemble_valid = false;
				bool reassemble_done = false;
				if ((rx_priv->rx_reassemble_cur_frags + 1) ==
				    rx_priv->rx_reassemble_total_frags) {
					if ((rx_priv->rx_reassemble_cur_len +
					     frag_len) ==
					    rx_priv->rx_reassemble_total_len) {
						reassemble_valid = true;
						reassemble_done = true;
					}
				} else {
					if (frag_len ==
					    aicwf_usb_max_pkt_size) {
						reassemble_valid = true;
					}
				}

				if (reassemble_valid) {
					memcpy((reassemble_skb->data +
						reassemble_skb->len),
					       skb->data, frag_len);
					skb_put(reassemble_skb, skb->len);
					rx_priv->rx_reassemble_cur_len +=
						frag_len;
			...
			if (pkt_check) {
				bool pkt_drop = false;
				u8 type = skb->data[2];
				u32 pkt_len = skb->data[0] |
					      (skb->data[1] << 8);
				if ((type & USB_TYPE_CFG) != USB_TYPE_CFG) {
					u32 pkt_total_len =
						pkt_len + RX_HWHRD_LEN;
					if ((pkt_total_len >
					     aicwf_usb_max_pkt_size) &&
					    (skb->len ==
					     aicwf_usb_max_pkt_size)) {
						...
						if (reassemble_skb) {
							memcpy(reassemble_skb
								       ->data,
							       skb->data,
							       skb->len);
							skb_put(reassemble_skb,
								skb->len);
							rx_priv->rx_reassemble_skb =
								reassemble_skb;
							rx_priv->rx_reassemble_total_len =
...
static void aicwf_usb_msg_rx_complete(struct urb *urb)
{
	...
	if (usb_dev->state == USB_UP_ST) {
		...
		if (rx_priv->rx_msg_reassemble_skb) {
			u32 frag_len = skb->len;
			struct sk_buff *reassemble_skb =
				rx_priv->rx_msg_reassemble_skb;
			bool reassemble_valid = false;
			bool reassemble_done = false;
			if ((rx_priv->rx_msg_reassemble_cur_frags + 1) ==
			    rx_priv->rx_msg_reassemble_total_frags) {
				if ((rx_priv->rx_msg_reassemble_cur_len +
				     frag_len) ==
				    rx_priv->rx_msg_reassemble_total_len) {
					reassemble_valid = true;
					reassemble_done = true;
				}
			} else {
			...
			if (reassemble_valid) {
				...
				if (reassemble_done) {
					...
					aicwf_usb_msg_rx_buf_put(usb_dev,
								 usb_buf);
					aicwf_usb_msg_rx_submit_all_urb_(
						usb_dev);
					return;
				}
			} else {
				usb_err("invalid frag msg pkt, len=%u/%u/%u, frags=%u/%u\n",
					frag_len,
					rx_priv->rx_msg_reassemble_cur_len,
					rx_priv->rx_msg_reassemble_cur_len,
					rx_priv->rx_msg_reassemble_cur_frags,
					rx_priv->rx_msg_reassemble_total_frags);
				aicwf_dev_skb_free(reassemble_skb);
			...
			if ((type & USB_TYPE_CFG) != USB_TYPE_CFG) {
				...
				if (type == USB_TYPE_CFG_CMD_RSP) {
					u32 pkt_total_len =
						ALIGN((pkt_len + 4), 4);
					if ((pkt_total_len >
					     AICWF_USB_MSG_MAX_PKT_SIZE) &&
					    (skb->len ==
					     AICWF_USB_MSG_MAX_PKT_SIZE)) {
						AICWFDBG(
							LOGINFO,
							"reassemble msg pkt, len=%u\n",
							pkt_total_len);
						struct sk_buff *reassemble_skb =
							__dev_alloc_skb(
								pkt_total_len,
		...
		if (!aicwf_rxframe_enqueue(usb_dev->dev, &rx_priv->msg_rxq,
					   skb)) {
				   ...
				   struct aicwf_usb_buf *usb_buf)
{
	...
	usb_fill_bulk_urb(usb_buf->urb, usb_dev->udev, usb_dev->bulk_in_pipe,
			  rx_buff->data, aicwf_rxbuff_size_get(),
			  aicwf_usb_rx_complete, usb_buf);
				   ...
				   struct aicwf_usb_buf *usb_buf)
{
	...
	if (aicwf_usb_rx_aggr) {
		skb = __dev_alloc_skb(AICWF_USB_AGGR_MAX_PKT_SIZE,
				      GFP_ATOMIC /*GFP_KERNEL*/);
	} else {
		skb = __dev_alloc_skb(aicwf_usb_max_pkt_size,
				      GFP_ATOMIC /*GFP_KERNEL*/);
	}
	if (!skb) {
		aicwf_usb_rx_buf_put(usb_dev, usb_buf);
	...
	if (aicwf_usb_rx_aggr) {
		usb_fill_bulk_urb(usb_buf->urb, usb_dev->udev,
				  usb_dev->bulk_in_pipe, skb->data,
				  AICWF_USB_AGGR_MAX_PKT_SIZE,
				  aicwf_usb_rx_complete, usb_buf);
	} else {
		usb_fill_bulk_urb(usb_buf->urb, usb_dev->udev,
				  usb_dev->bulk_in_pipe, skb->data,
				  aicwf_usb_max_pkt_size, aicwf_usb_rx_complete,
				  usb_buf);
	}
				       ...
				       struct aicwf_usb_buf *usb_buf)
{
	...
	usb_fill_bulk_urb(usb_buf->urb, usb_dev->udev, usb_dev->msg_in_pipe,
...
int aicwf_usb_send_pkt(struct aic_usb_dev *usb_dev, u8 *buf, uint buf_len)
{
	...
	print_hex_dump(KERN_ERR, "buf  ", DUMP_PREFIX_NONE, 16, 1, &buf[0], 32,
		       false);
...
int aicwf_usb_aggr(struct aicwf_tx_priv *tx_priv, struct sk_buff *pkt)
{
	...
	u32 curr_len = 0;
	int allign_len = 0;
	u32 data_len = (pkt->len - sizeof(struct rwnx_txhdr) +
...
static void aicwf_usb_tx_process(struct aic_usb_dev *usb_dev)
{
	...
	while (!list_empty(&usb_dev->tx_post_list)) {
		...
		if (ret) {
			AICWFDBG(
				LOGERROR,
				"aicwf_usb_bus_tx usb_submit_urb FAILED err:%d\n",
				ret);
#ifdef CONFIG_USB_TX_AGGR
			aicwf_usb_tx_queue(usb_dev, &usb_dev->tx_post_list,
					   usb_buf, &usb_dev->tx_post_count,
					   &usb_dev->tx_post_lock);
			break;
#else
			goto fail;
	...
	while (1) {
		...
		if (!wait_for_completion_interruptible(&bus->bustx_trgg)) {
			...
			if ((usbdev->tx_post_count > 0) ||
			    !aicwf_is_framequeue_empty(&usbdev->tx_priv->txq))
#else
			if (usbdev->tx_post_count > 0)
#endif
...
int usb_msg_busrx_thread(void *data)
{
	...
	while (1) {
		...
		if (!wait_for_completion_interruptible(
			    &bus_if->msg_busrx_trgg)) {
			if (bus_if->state == BUS_DOWN_ST) {
				AICWFDBG(LOGINFO,
					 "usb msg busrx thread will to stop\n");
				break;
			}
...
static int aicwf_usb_bus_txmsg(struct device *dev, u8 *buf, u32 len)
{
	...
	if (usb_dev->msg_out_pipe) {
		usb_fill_bulk_urb(usb_dev->msg_out_urb, usb_dev->udev,
				  usb_dev->msg_out_pipe, buf, len,
				  (usb_complete_t)aicwf_usb_send_msg_complete,
				  usb_dev);
	} else {
		usb_fill_bulk_urb(usb_dev->msg_out_urb, usb_dev->udev,
...
static void aicwf_usb_free_urb(struct list_head *q, spinlock_t *qlock)
{
	...
	list_for_each_entry_safe(usb_buf, tmp, q, list) {
		...
		if (usb_buf->data_buf) {
			...
			usb_free_coherent(usb_buf->usbdev->udev, DATA_BUF_MAX,
					  usb_buf->data_buf,
					  usb_buf->data_dma_trans_addr);
#else
			usb_buffer_free(usb_buf->usbdev->udev, DATA_BUF_MAX,
					usb_buf->data_buf,
					usb_buf->data_dma_trans_addr);
#endif
...
// alloc dma buf
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35))
		usb_buf->data_buf = usb_alloc_coherent(
			usb_dev->udev, DATA_BUF_MAX,
			(in_interrupt() ? GFP_ATOMIC : GFP_KERNEL),
			&usb_buf->data_dma_trans_addr);
#else
		usb_buf->data_buf = usb_buffer_alloc(
			usb_dev->udev, DATA_BUF_MAX,
			(in_interrupt() ? GFP_ATOMIC : GFP_KERNEL),
			&usb_buf->data_dma_trans_addr);
#endif
...
err:
	aicwf_usb_free_urb(&usb_dev->msg_rx_free_list,
			   &usb_dev->msg_rx_free_lock);
	return -ENOMEM;
}
#endif
...
static void usb_tx_flow_ctrl(struct rwnx_txhdr *txhdr,
			     struct aic_usb_dev *usb_dev,
			     struct rwnx_hw *rwnx_hw)
{
	...
	if (sta_idx < NX_REMOTE_STA_MAX &&
	    !(txhdr->sw_hdr->desc.host.flags & TXU_CNTRL_MGMT)) {
		struct rwnx_vif *vif = NULL;
		...
		if (RWNX_VIF_TYPE(vif) == NL80211_IFTYPE_AP) {
			if ((atomic_read(&rwnx_hw->sta_flowctrl[sta_idx]
						  .tx_pending_cnt) >=
				     AICWF_USB_FC_PERSTA_HIGH_WATER &&
			     rwnx_hw->sta_flowctrl[sta_idx].flowctrl == 0) ||
...
#error "CONFIG_USB_NO_TRANS_DMA_MAP not supported"
	...
	if (!IS_ALIGNED((unsigned long)buf, align_param)) {
		usb_buf->usb_align_data = (u8 *)kmalloc(
			sizeof(u8) * buf_len + align_param, GFP_ATOMIC);
		if (usb_buf->usb_align_data) {
			align = ((unsigned long)(usb_buf->usb_align_data)) &
				(align_param - 1);
			buf_align =
				usb_buf->usb_align_data + (align_param - align);
...
flow_ctrl:
	...
	if (usb_dev->tx_free_count < AICWF_USB_TX_LOW_WATER) {
		AICWFDBG(
			LOGDEBUG,
			"usb_dev->tx_free_count < AICWF_USB_TX_LOW_WATER:%d\r\n",
			usb_dev->tx_free_count);
		usb_dev->tbusy = true;
		aicwf_usb_tx_flowctrl(usb_dev->rwnx_hw, true);
	}
...
static void aicwf_usb_deinit(struct aic_usb_dev *usbdev)
{
	...
	if (usbdev->msg_in_pipe) {
		...
		aicwf_usb_free_urb(&usbdev->msg_rx_free_list,
				   &usbdev->msg_rx_free_lock);
	}
#endif
		...
		if (usb_endpoint_dir_in(endpoint) &&
			...
			if (!usb_dev->bulk_in_pipe) {
				...
				if (usb_dev->chipid != PRODUCT_ID_AIC8801) {
					usb_dev->msg_in_pipe = usb_rcvbulkpipe(
						usb, endpoint_num);
				}
			}
#endif
...
static int aicwf_usb_chipmatch(struct aic_usb_dev *usb_dev, u16_l vid,
			       u16_l pid)
{
			   ...
			   const struct usb_device_id *id)
{
	...
	AICWFDBG(LOGDEBUG, "%s usb_dev:%d usb_tx_buf:%d usb_rx_buf:%d\r\n",
		 __func__, (int)sizeof(struct aic_usb_dev),
	...
	usb_dev->usb_tx_buf =
		vmalloc(sizeof(struct aicwf_usb_buf) * AICWF_USB_TX_URBS);
	usb_dev->usb_rx_buf =
		vmalloc(sizeof(struct aicwf_usb_buf) * AICWF_USB_RX_URBS);
	...
	if (!usb_dev->usb_tx_buf || !usb_dev->usb_rx_buf) {
		...
		AICWFDBG(LOGERROR,
			 "%s usb_tx_buf or usb_rx_buf vmalloc fail\r\n",
			 __func__);
		return -ENOMEM;
	}

	memset(usb_dev->usb_tx_buf, 0,
	       (int)(sizeof(struct aicwf_usb_buf) * AICWF_USB_TX_URBS));

	memset(usb_dev->usb_rx_buf, 0,
	       (int)(sizeof(struct aicwf_usb_buf) * AICWF_USB_RX_URBS));
...
static struct usb_device_id aicwf_usb_id_table[] = {
	...
	{ USB_DEVICE_AND_INTERFACE_INFO(
		USB_VENDOR_ID_AIC, USB_PRODUCT_ID_AIC8801, 0xff, 0xff, 0xff) },
	{ USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_AIC,
					USB_PRODUCT_ID_AIC8800D81, 0xff, 0xff,
					0xff) },
	{ USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_AIC,
					USB_PRODUCT_ID_AIC8800D41, 0xff, 0xff,
					0xff) },
	{ USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_AIC,
					USB_PRODUCT_ID_AIC8800DC, 0xff, 0xff,
					0xff) },
	{ USB_DEVICE(USB_VENDOR_ID_AIC, USB_PRODUCT_ID_AIC8800DW) },
	{ USB_DEVICE_AND_INTERFACE_INFO(USB_VENDOR_ID_AIC_V2,
					USB_PRODUCT_ID_AIC8800D81X2, 0xff, 0xff,
					0xff) },
...
void aicwf_usb_exit(void)
{
	...
	AICWFDBG(LOGDEBUG, "%s in_interrupt:%d in_softirq:%d in_atomic:%d\r\n",
		 __func__, (int)in_interrupt(), (int)in_softirq(),
		 (int)in_atomic());

	do {
		AICWFDBG(
			LOGINFO,
			"aicwf_deinit_atomic is busy. waiting for 500ms retry:%d \r\n",
			retry);
		mdelay(500);
		retry--;
		if (retry == 0) {
	...
	atomic_set(&aicwf_deinit_atomic, 0);
	if (down_timeout(&aicwf_deinit_sem, msecs_to_jiffies(SEM_TIMOUT)) !=
	    0) {
	...
	if (!g_rwnx_plat || !g_rwnx_plat->enabled) {
		AICWFDBG(LOGINFO,
			 "g_rwnx_plat is not ready. waiting for 500ms\r\n");
		mdelay(500);
	}
