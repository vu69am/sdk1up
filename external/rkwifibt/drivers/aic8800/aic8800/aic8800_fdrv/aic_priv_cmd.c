				     ...
				     char *argv[], char *command)
{
	...
	if ((dev->chipid == PRODUCT_ID_AIC8800DC) ||
				     ...
				     char *argv[], char *command)
{
	...
	if (func == 0) { // read cur
		...
		if ((argc > 4) && ((dev->chipid == PRODUCT_ID_AIC8800D81) ||
			...
			rwnx_send_rftest_req(rwnx_hw, RDWR_PWROFST, sizeof(buf),
			   ...
			   ((dev->chipid == PRODUCT_ID_AIC8801) ||
			    (dev->chipid == PRODUCT_ID_AIC8800DW) ||
			    (dev->chipid == PRODUCT_ID_AIC8800DC))) {
	...
	if ((dev->chipid == PRODUCT_ID_AIC8800DC) ||
	    (dev->chipid == PRODUCT_ID_AIC8800DW)) { // 3 = 3 (2.4g)
		res_len = 3;
	} else if (dev->chipid ==
		   PRODUCT_ID_AIC8800D81) { // 3 * 2 (2.4g) + 3 * 6 (5g)
		res_len = 3 * 3 + 3 * 6;
	} else if ((dev->chipid == PRODUCT_ID_AIC8800D81X2) ||
		   (dev->chipid == PRODUCT_ID_AIC8800D89X2)) { // ant0/1
		res_len = (3 * 3 + 3 * 6) * 2;
	} else {
		res_len = 3 + 4;
	}
					   ...
					   char *argv[], char *command)
{
	...
	if ((dev->chipid == PRODUCT_ID_AIC8800DC) ||
	    (dev->chipid == PRODUCT_ID_AIC8800DW)) { // 6 = 3 (2.4g) * 2
		res_len = 3 * 2;
	} else if ((dev->chipid == PRODUCT_ID_AIC8800D81) ||
		   (dev->chipid == PRODUCT_ID_AIC8800D81X2) ||
		   (dev->chipid ==
		    PRODUCT_ID_AIC8800D89X2)) { // 3 * 2 (2.4g) + 3 * 6 (5g)
		res_len = (3 * 3 + 3 * 6) * 2;
	} else { // 7 = 3(2.4g) + 4(5g)
		res_len = 3 + 4;
	}
					...
					char *argv[], char *command)
{
	...
	if (!state) {
		AICWFDBG(LOGINFO,
			 "cap= 0x%x, cap_fine= 0x%x, freq_ofst= %d Hz\n",
...
#define PRIVATE_COMMAND_DEF_LEN 4096
	...
	if (!strncasecmp(command, CMD_SET_COUNTRY, strlen(CMD_SET_COUNTRY)) &&
		...
		ret = 0;
		goto exit;
	} else if (!strncasecmp(command, CMD_SET_TESTMODE,
				strlen(CMD_SET_TESTMODE))) {
		if (g_rwnx_plat && g_rwnx_plat->usbdev->rwnx_hw) {
			if (g_rwnx_plat->usbdev->chipid ==
				    PRODUCT_ID_AIC8800DW ||
			    (g_rwnx_plat->usbdev->chipid ==
		...
		ret = 0;
		goto exit;
	} else if (!strncasecmp(command, CMD_SET_BLE_WAKE,
				strlen(CMD_SET_BLE_WAKE))) {
		if (g_rwnx_plat && g_rwnx_plat->usbdev->rwnx_hw) {
			set_testmode(5);
			rwnx_send_reboot(g_rwnx_plat->usbdev->rwnx_hw);
		}
