...
static ssize_t rwnx_dbgfs_rhd_read(struct file *file, char __user *user_buf,
				   size_t count, loff_t *ppos)
{
	...
	read = simple_read_from_buffer(user_buf, count, ppos, dump->rhd_mem,
...
static ssize_t rwnx_dbgfs_rbd_read(struct file *file, char __user *user_buf,
				   size_t count, loff_t *ppos)
{
	struct rwnx_hw *priv = file->private_data;
	struct dbg_debug_dump_tag *dump = priv->dbgdump_elem.buf.addr;
	...
	read = simple_read_from_buffer(user_buf, count, ppos, dump->rbd_mem,
...
static ssize_t rwnx_dbgfs_thd0_read(struct file *file, char __user *user_buf,
				    size_t count, loff_t *ppos)
{
...
static ssize_t rwnx_dbgfs_thd1_read(struct file *file, char __user *user_buf,
				    size_t count, loff_t *ppos)
{
...
static ssize_t rwnx_dbgfs_thd2_read(struct file *file, char __user *user_buf,
				    size_t count, loff_t *ppos)
{
...
static ssize_t rwnx_dbgfs_thd3_read(struct file *file, char __user *user_buf,
				    size_t count, loff_t *ppos)
{
...
#if (NX_TXQ_CNT == 5)
static ssize_t rwnx_dbgfs_thd4_read(struct file *file, char __user *user_buf,
				    size_t count, loff_t *ppos)
{
					...
					loff_t *ppos)
{
	...
	read = simple_read_from_buffer(user_buf, count, ppos, dump->la_mem,
...
static ssize_t rwnx_dbgfs_hwdiags_read(struct file *file, char __user *user_buf,
				       size_t count, loff_t *ppos)
{
	struct rwnx_hw *priv = file->private_data;
	struct dbg_debug_dump_tag *dump = priv->dbgdump_elem.buf.addr;
...
static ssize_t rwnx_dbgfs_swdiags_read(struct file *file, char __user *user_buf,
				       size_t count, loff_t *ppos)
{
	struct rwnx_hw *priv = file->private_data;
	struct dbg_debug_dump_tag *dump = priv->dbgdump_elem.buf.addr;
...
static ssize_t rwnx_dbgfs_error_read(struct file *file, char __user *user_buf,
				     size_t count, loff_t *ppos)
{
	struct rwnx_hw *priv = file->private_data;
	struct dbg_debug_dump_tag *dump = priv->dbgdump_elem.buf.addr;
...
static ssize_t rwnx_dbgfs_rxdesc_read(struct file *file, char __user *user_buf,
				      size_t count, loff_t *ppos)
{
	struct rwnx_hw *priv = file->private_data;
	struct dbg_debug_dump_tag *dump = priv->dbgdump_elem.buf.addr;
	...
	ret = scnprintf(buf, min_t(size_t, sizeof(buf) - 1, count),
			"%08X\n%08X\n", dump->dbg_info.rhd, dump->dbg_info.rbd);
...
static ssize_t rwnx_dbgfs_txdesc_read(struct file *file, char __user *user_buf,
				      size_t count, loff_t *ppos)
{
	struct rwnx_hw *priv = file->private_data;
	struct dbg_debug_dump_tag *dump = priv->dbgdump_elem.buf.addr;
...
static void rwnx_um_helper_work(struct work_struct *ws)
{
	struct rwnx_debugfs *rwnx_debugfs =
		container_of(ws, struct rwnx_debugfs, helper_work);
