...
int aicwf_bus_init(uint bus_hdrlen, struct device *dev)
{
	...
	bus_if->cmd_buf =
		usb_alloc_coherent(bus_if->bus_priv.usb->udev, CMD_BUF_MAX,
				   (in_interrupt() ? GFP_ATOMIC : GFP_KERNEL),
				   &bus_if->bus_priv.usb->cmd_dma_trans_addr);
#else
	bus_if->cmd_buf =
		usb_buffer_alloc(bus_if->bus_priv.usb->udev, CMD_BUF_MAX,
				 (in_interrupt() ? GFP_ATOMIC : GFP_KERNEL),
				 &bus_if->bus_priv.usb->cmd_dma_trans_addr);
#endif
#else
	...
	bus_if->bustx_thread = kthread_run(sdio_bustx_thread, (void *)bus_if,
					   "aicwf_bustx_thread");
	bus_if->busrx_thread = kthread_run(
		sdio_busrx_thread, (void *)bus_if->bus_priv.sdio->rx_priv,
		"aicwf_busrx_thread");
#endif
#ifdef AICWF_USB_SUPPORT
	bus_if->busrx_thread = kthread_run(
		usb_busrx_thread, (void *)bus_if->bus_priv.usb->rx_priv,
		"aicwf_busrx_thread");
	...
	{
		AICWFDBG(LOGINFO,
			 "%s waiting for rx/tx thread init finish \r\n",
			 __func__);
		msleep(100);
	}
	...
	if (bus_if->bus_priv.usb->msg_in_pipe) {
		bus_if->msg_busrx_thread =
			kthread_run(usb_msg_busrx_thread,
...
#if defined CONFIG_USB_SUPPORT && defined CONFIG_USB_NO_TRANS_DMA_MAP
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35))
		usb_free_coherent(bus_if->bus_priv.usb->udev, CMD_BUF_MAX,
				  bus_if->cmd_buf,
				  bus_if->bus_priv.usb->cmd_dma_trans_addr);
#else
		usb_buffer_free(bus_if->bus_priv.usb->udev, CMD_BUF_MAX,
				bus_if->cmd_buf,
				bus_if->bus_priv.usb->cmd_dma_trans_addr);
#endif
#else
...
#endif
#endif
			rwnx_rxdataind_aicwf(rx_priv->usbdev->rwnx_hw, skb,
					     (void *)rx_priv);
...
//#endif
	...
	if (aicwf_usb_rx_aggr) {
		while (1) {
			...
			if (!rx_priv->rxq.qcnt) {
				usb_info("no more rxdata\n");
				spin_unlock_irqrestore(&rx_priv->rxqlock,
						       flags);
				break;
			}
			...
			while (aicwf_another_ptk_1(buffer)) {
				...
				if ((data[2] & USB_TYPE_CFG) !=
					...
					skb_put(skb_inblock, aggr_len);
					memcpy(skb_inblock->data, data,
					       aggr_len);
					rwnx_rxdataind_aicwf(
						rx_priv->usbdev->rwnx_hw,
					...
					buffer->read =
						buffer->read + (adjust_len + 4);
					buffer->len -= (adjust_len + 4);
					kfree(msg);
				}
			}
			aicwf_prealloc_rxbuff_free(buffer,
						   &rx_priv->rxbuff_lock);
			if (rx_urb_sched) {
	...
	if (aicwf_usb_rx_aggr) {
		while (1) {
			...
			while (aicwf_another_ptk(skb)) {
				...
				data = skb->data;
				pkt_len =
					(*skb->data | (*(skb->data + 1) << 8));
				//printk("p:%d, s:%d , %x\n", pkt_len, skb->len, data[2]);
#ifndef CONFIG_USB_RX_REASSEMBLE
				if (pkt_len > skb->len) {
					AICWFDBG(
						LOGERROR,
						"%s pkt_len:%d skb->len:%d\r\n",
						__func__, pkt_len, skb->len);
					//dev_kfree_skb(skb);
					//atomic_dec(&rx_priv->rx_cnt);
					//continue;
				...
				if ((skb->data[2] & USB_TYPE_CFG) !=
					...
					msg = kmalloc(
						aggr_len + 4,
						GFP_KERNEL); //hdr(4)+config(aggrlen)
					if (msg == NULL) {
						txrx_err(
							"no more space for msg!\n");
						aicwf_dev_skb_free(skb);
...
int aicwf_process_msg_rxframes(struct aicwf_rx_priv *rx_priv)
{
	...
	while (1) {
		...
		if ((skb->data[2] & USB_TYPE_CFG) !=
			...
			if ((*(msg + 2) & 0x7f) == USB_TYPE_CFG_PRINT)
				rwnx_rx_handle_print(rx_priv->usbdev->rwnx_hw,
						     msg + 4, aggr_len);
