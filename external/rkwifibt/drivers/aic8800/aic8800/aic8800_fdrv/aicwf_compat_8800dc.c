...
u32 patch_tbl_wifisetting[][2] = {
	...
	{ 0x0100,
	  0x03021714 }, //usb fc params(rx msg fc recover, rx msg fc trigger, wifi fc recover, wifi fc trigger)
	{ 0x0120,
	  0x140A0100 }, //usb agg tx params(total cnt, aggr cnt, out en, global out nak)
...
#ifdef CONFIG_LOAD_BT_PATCH_IN_FDRV
int rwnx_get_patch_addr_from_patch_table(struct rwnx_hw *rwnx_hw,
					 char *filename,
					 uint32_t *fw_patch_base_addr)
{
	int size;
	int ret = 0;
	...
	/* load aic firmware */
	size = rwnx_request_firmware_common(rwnx_hw, (u32 **)&rawdata,
					    filename);
...
int rwnx_patch_table_load(struct rwnx_hw *rwnx_hw,
			  struct aicbt_patch_table *_head)
{
	...
	for (p = head; p != NULL; p = p->next) {
		...
		for (i = 0; i < p->len; i++) {
			ret = rwnx_send_dbg_mem_write_req(rwnx_hw, *data,
							  *(data + 1));
			if (ret != 0)
				return ret;
			data += 2;
...
int aicwf_patch_table_load(struct rwnx_hw *rwnx_hw, char *filename)
{
	...
	if (!err && (i < size)) {
		err = rwnx_send_dbg_mem_block_write_req(
			rwnx_hw, fmacfw_patch_tbl_8800dc_u02_describe_base,
			fmacfw_patch_tbl_8800dc_u02_describe_size + 4, dst);
		if (err) {
			printk("write describe information fail \n");
		}

		describle = kzalloc(fmacfw_patch_tbl_8800dc_u02_describe_size,
				    GFP_KERNEL);
		memcpy(describle, &dst[1],
		       fmacfw_patch_tbl_8800dc_u02_describe_size);
	...
	if (!err && (i < size)) {
		for (i = (128 / 4); i < (size / 4); i += 2) {
			AICWFDBG(LOGERROR, "patch_tbl:  %x  %x\n", dst[i],
				 dst[i + 1]);
			err = rwnx_send_dbg_mem_write_req(rwnx_hw, dst[i],
							  dst[i + 1]);
		}
		if (err) {
...
int aicwf_patch_var_config_8800dc(struct rwnx_hw *rwnx_hw)
{
	...
	if (wr_ext_flags_val != rd_ext_flags_val) {
		ret = rwnx_send_dbg_mem_write_req(rwnx_hw, rd_ext_flags_addr,
						  wr_ext_flags_val);
		if (ret) {
...
void aicwf_patch_config_8800dc(struct rwnx_hw *rwnx_hw)
{
	...
	if (testmode == 0) {
		...
		u32 patch_tbl_wifisetting_num =
			sizeof(patch_tbl_wifisetting) / sizeof(u32) / 2;
		...
		if (adap_test) {
			adap_patch_num =
				sizeof(adaptivity_patch_tbl) / sizeof(u32) / 2;
			for (cnt = 0; cnt < adap_patch_num; cnt++) {
				if ((ret = rwnx_send_dbg_mem_write_req(
					     rwnx_hw,
					     wifisetting_cfg_addr +
						     adaptivity_patch_tbl[cnt]
									 [0],
			       ...
			       struct mm_set_rf_calib_cfm *cfm)
{
	...
	if (testmode == FW_NORMAL_MODE) {
#if !defined(CONFIG_EXT_FEM_8800DCDW)
		if (IS_CHIP_ID_H()) {
			if ((ret = rwnx_send_rf_config_req(
				     rwnx_hw, 0, 1,
...
#else /* #ifdef CONFIG_EXT_FEM_8800DCDW */
		{
			ret = rwnx_send_rf_config_req(
				rwnx_hw, 0, 1,
				(u8_l *)wifi_txgain_table_24g_8800dcdw_femkct,
				128);
			if (ret) {
				return -1;
			}
			ret = rwnx_send_rf_config_req(
				rwnx_hw, 16, 1,
				(u8_l *)wifi_txgain_table_24g_1_8800dcdw_femkct,
				128);
			if (ret) {
				return -1;
			}
		...
		if ((ret = rwnx_send_rf_config_req(
			     rwnx_hw, 0, 0,
			     (u8_l *)wifi_rxgain_table_24g_20m_8800dcdw, 256)))
			return -1;

		if ((ret = rwnx_send_rf_config_req(
			     rwnx_hw, 32, 0,
			     (u8_l *)wifi_rxgain_table_24g_40m_8800dcdw, 256)))
			return -1;
		...
		if (chip_sub_id >= 1) {
			...
			if (dpd_res.bit_mask[1]) {
				ret = aicwf_dpd_result_apply_8800dc(rwnx_hw,
								    &dpd_res);
				if (ret) {
					AICWFDBG(LOGINFO,
			...
			if (loft_res_local.bit_mask[1]) {
				ret = aicwf_loft_result_apply_8800dc(
...
int aicwf_plat_patch_load_8800dc(struct rwnx_hw *rwnx_hw)
{
	...
	if (chip_sub_id == 0) {
		...
		ret = rwnx_get_patch_addr_from_patch_table(
			rwnx_hw, FW_PATCH_TABLE_NAME_U02,
			&fw_ram_patch_base_addr);
		//bt patch
		printk("%s %x\n", __func__, fw_ram_patch_base_addr);
		ret = rwnx_plat_bin_fw_upload_2(
			rwnx_hw, FW_8800DC_U02_ADID_ADDR, FW_ADID_BASE_NAME);
		//fw_ram_patch_base_addr, FW_PATCH_BASE_NAME_U02
		ret = rwnx_plat_bin_fw_upload_2(rwnx_hw, fw_ram_patch_base_addr,
						FW_PATCH_BASE_NAME_U02);
		//bt patch table
		ret = rwnx_patch_table_download(rwnx_hw,
						FW_PATCH_TABLE_NAME_U02);
#endif
		//wifi patch
		ret = rwnx_plat_bin_fw_upload_2(
			rwnx_hw, ROM_FMAC_PATCH_ADDR,
		...
		//fw_ram_patch_base_addr, FW_PATCH_BASE_NAME_U02
		ret = rwnx_plat_bin_fw_upload_2(rwnx_hw, fw_ram_patch_base_addr,
						FW_PATCH_BASE_NAME_U02H);
		//bt patch table
		ret = rwnx_patch_table_download(rwnx_hw,
						FW_PATCH_TABLE_NAME_U02H);
#endif
		//wifi patch
		ret = rwnx_plat_bin_fw_upload_2(
			rwnx_hw, ROM_FMAC_PATCH_ADDR,
...
int aicwf_misc_ram_valid_check_8800dc(struct rwnx_hw *rwnx_hw, int *valid_out)
{
	...
	for (i = 0; i < ram_word_cnt; i++) {
		ret = rwnx_send_dbg_mem_read_req(rwnx_hw, ram_base_addr + i * 4,
						 &cfm);
		if (ret) {
...
int aicwf_dpd_calib_8800dc(struct rwnx_hw *rwnx_hw, rf_misc_ram_lite_t *dpd_res)
{
	...
	{ // read dpd res
		...
		uint32_t ram_base_addr, ram_word_cnt;
		int i;
		ret = rwnx_send_dbg_mem_read_req(rwnx_hw, cfg_base + 0x14,
						 &cfm);
		if (ret) {
			AICWFDBG(LOGERROR, "rf misc ram[0x%x] rd fail: %d\n",
				 cfg_base + 0x14, ret);
				  ...
				  rf_misc_ram_lite_t *dpd_res)
{
	...
	if ((ret = rwnx_send_dbg_mem_read_req(rwnx_hw, cfg_base + 0x14,
					      &cfm))) {
		AICWFDBG(LOGERROR, "rf misc ram[0x%x] rd fail: %d\n",
			 cfg_base + 0x14, ret);
		return ret;
	...
	ram_byte_cnt = MEMBER_SIZE(rf_misc_ram_t, bit_mask) +
		       MEMBER_SIZE(rf_misc_ram_t, reserved);
	ret = rwnx_send_dbg_mem_block_write_req(rwnx_hw, ram_base_addr,
						ram_byte_cnt,
