...
#define CAPTION \
	"status: L=in hwq list, F=stop full, P=stop sta PS, V=stop vif PS, C=stop channel, S=stop CSA, M=stop MU"
				       ...
				       size_t count, loff_t *ppos)
{
	...
	buf = (char *)kmalloc(sizeof(char) * ((SCAN_CHANNEL_MAX + 1) * 43),
			      GFP_KERNEL);
				      ...
				      size_t count, loff_t *ppos)
{
	...
	if ((priv->debugfs.fw_log.buf.start + priv->debugfs.fw_log.buf.size) >=
		...
		priv->debugfs.fw_log.buf.start =
			priv->debugfs.fw_log.buf.start +
			priv->debugfs.fw_log.buf.size - not_cpy;
	}
	...
	printk("nb_cpy=%lu, not_cpy=%lu, start=%p, end=%p\n",
	       (long unsigned int)nb_cpy, (long unsigned int)not_cpy,
				       ...
				       size_t count, loff_t *ppos)
{
	...
	if (oper == 0) {
		...
		printk("[0x%x] = [0x%x]\n", mem_read_cfm.memaddr,
		       mem_read_cfm.memdata);
	} else if (oper == 1) {
		ret = rwnx_send_dbg_mem_read_req(priv, addr, &mem_read_cfm);
						...
						size_t count, loff_t *ppos)
{
	...
	ret = sscanf(buf, "%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x",
		     &hwconfig_id, &addr[0], &addr[1], &addr[2], &addr[3],
		     &addr[4], &addr[5], &addr[6], &addr[7], &addr[8], &addr[9],
		     &addr[10], &addr[11], &addr[12], &addr[13]);
	if (ret > 15) {
		...
		case 0:
			...
			ret = rwnx_send_vendor_hwconfig_req(priv, hwconfig_id,
							    addr, NULL);
		...
		case 4: // BWMODE_REQ
			if (ret != 2) {
				...
				ret = rwnx_send_vendor_hwconfig_req(
					priv, hwconfig_id, addr, NULL);
		...
		case 5: // CHIP_TEMP_GET_REQ
			if (ret != 1) {
				...
				ret = rwnx_send_vendor_hwconfig_req(
					priv, hwconfig_id, addr, addr_out);
	...
	if (ret > 3) {
		...
		case 0: // BCN_CFG_REQ
			if (ret != 2) {
				...
				ret = rwnx_send_vendor_swconfig_req(
					priv, swconfig_id, addr, addr_out);
					...
					size_t count, loff_t *ppos)
{
	...
	if ((ret > 3) || (ret < 1)) {
		...
		if (roc_start) {
			...
			chan->max_power = 18;
			ret = rwnx_send_roc(priv, vif, chan, duration,
					    &roc_cfm);
			if (ret) {
				printk("rwnx_send_roc fail: %d\n", ret);
			} else {
				printk("roc_cfm: opcode=%x, st=%d, idx=%d\n",
				       roc_cfm.op_code, roc_cfm.status,
				       roc_cfm.chan_ctxt_index);
			}
		} else {
		      ...
		      int sgi, int pre, int *r_idx)
{
	...
	if (format < FORMATMOD_HT_MF) {
		...
		res += scnprintf(&buf[res], size - res,
				 "HE%d/GI%s%*cMCS%d/%1d%*c", 20 * (1 << bw),
				 he_gi[sgi], bw > 2 ? 4 : 5, ' ', mcs, nss + 1,
				 mcs > 9 ? 1 : 2, ' ');
	} else {
		if (r_idx) {
...
static void idx_to_rate_cfg1(unsigned int formatmod, unsigned int mcs,
			     unsigned int nss, unsigned int bwTx,
					...
					loff_t *ppos)
{
	...
	sscanf(file->f_path.dentry->d_parent->d_iname,
	       "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2],
	       &mac[3], &mac[4], &mac[5]);
	//if (mac == NULL)
	//    return 0;
						  ...
						  size_t count, loff_t *ppos)
{
	...
	if ((formatmod > 6) || (mcs > 11) || (nss > 8) || (bwTx > 6) ||
	    (gi > 3)) {
	...
	if ((fixed_rate_idx < 0) ||
		...
		idx_to_rate_cfg1(formatmod, mcs, nss, bwTx, gi, &rate_config,
				 NULL);
	}

	printk("formatModTx=%u mcsIndexTx=%u bwTx=%u giAndPreTypeTx=%u\n",
	       r_cfg->formatModTx, r_cfg->mcsIndexTx, r_cfg->bwTx,
	       r_cfg->giAndPreTypeTx);
				       ...
				       size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < rate_stats->size; i++) {
		if (rate_stats->table[i]) {
			union rwnx_rate_ctrl_info rate_config;
			int percent =
				(rate_stats->table[i] * 1000) / rate_stats->cpt;
			int p;
			int ru_size;
...
static void rwnx_rc_stat_work(struct work_struct *ws)
{
	...
	if (rwnx_debugfs->dir_sta[sta_idx] == NULL) {
		...
		if (sta->sta_idx >= NX_REMOTE_STA_MAX) {
			...
			scnprintf(sta_name, sizeof(sta_name), "%pM",
				  sta->mac_addr);
		}
		...
		file = debugfs_create_file("fixed_rate_idx", S_IWUSR, dir_sta,
					   rwnx_hw,
					   &rwnx_dbgfs_rc_fixed_rate_idx_ops);
		if (IS_ERR_OR_NULL(file))
			goto error_after_dir;

		file = debugfs_create_file("rx_rate", S_IRUSR | S_IWUSR,
					   dir_sta, rwnx_hw,
		...
		rate_stats->table = kzalloc(
			nb_rx_rate * sizeof(rate_stats->table[0]), GFP_KERNEL);
		if (!rate_stats->table)
			goto error_after_dir;
		...
		list_for_each_entry_safe(rc_cfg, next,
					 &rwnx_debugfs->rc_config_save, list) {
			if (jiffies_to_msecs(jiffies - rc_cfg->timestamp) >
			    RC_CONFIG_DUR) {
				list_del(&rc_cfg->list);
				kfree(rc_cfg);
			} else if (!memcmp(rc_cfg->mac_addr, sta->mac_addr,
					   ETH_ALEN)) {
		...
		if (rwnx_debugfs->rc_config[sta_idx] >= 0) {
			...
			if (rc_cfg) {
				...
				memcpy(rc_cfg->mac_addr, sta->mac_addr,
				       ETH_ALEN);
