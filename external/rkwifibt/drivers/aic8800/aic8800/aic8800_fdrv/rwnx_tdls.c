					...
					struct sk_buff *skb)
{
	...
	tf = (void *)skb_put(skb, sizeof(struct ieee80211_tdls_data) -
					  sizeof(tf->u));
	...
	case WLAN_TDLS_SETUP_REQUEST:
		...
		tf->u.setup_req.capability = cpu_to_le16(
			rwnx_get_tdls_sta_capab(rwnx_vif, status_code));
		break;
	...
	case WLAN_TDLS_SETUP_RESPONSE:
		...
		tf->u.setup_resp.capability = cpu_to_le16(
			rwnx_get_tdls_sta_capab(rwnx_vif, status_code));
		break;
				 ...
				 u16 status_code, struct sk_buff *skb)
{
	...
	case WLAN_PUB_ACTION_TDLS_DISCOVER_RES:
		...
		mgmt->u.action.u.tdls_discover_resp.capability = cpu_to_le16(
			rwnx_get_tdls_sta_capab(rwnx_vif, status_code));
		break;
	default:
		return -EINVAL;
				 ...
				 u16 prot_mode)
{
	...
	*pos++ = sizeof(struct ieee80211_ht_operation);
	...
	case NL80211_CHAN_WIDTH_40:
		if (rwnx_vif->sta.ap->center_freq1 >
				  ...
				  u16 prot_mode)
{
	...
	*pos++ = sizeof(struct ieee80211_vht_operation);
	...
	case NL80211_CHAN_WIDTH_80:
		vht_oper->chan_width =
			IEEE80211_VHT_CHANWIDTH_80MHZ; // Channel Width
		CCFS0(vht_oper) = ieee80211_frequency_to_channel(
			rwnx_vif->sta.ap
				->center_freq); // Channel Center Frequency Segment 0
		CCFS1(vht_oper) =
			0; // Channel Center Frequency Segment 1 (N.A.)
		break;
	case NL80211_CHAN_WIDTH_160:
		vht_oper->chan_width =
			IEEE80211_VHT_CHANWIDTH_160MHZ; // Channel Width
		CCFS0(vht_oper) = ieee80211_frequency_to_channel(
			rwnx_vif->sta.ap
				->center_freq); // Channel Center Frequency Segment 0
		CCFS1(vht_oper) =
	...
	case NL80211_CHAN_WIDTH_80P80:
		...
		CCFS0(vht_oper) = ieee80211_frequency_to_channel(
			rwnx_vif->sta.ap
				->center_freq1); // Channel Center Frequency Segment 0
		CCFS1(vht_oper) = ieee80211_frequency_to_channel(
			rwnx_vif->sta.ap
				->center_freq2); // Channel Center Frequency Segment 1
		break;
	default:
			      ...
			      struct cfg80211_chan_def *chandef)
{
	...
	case WLAN_PUB_ACTION_TDLS_DISCOVER_RES:
		if (status_code == 0)
			rwnx_tdls_add_setup_start_ies(rwnx_hw, rwnx_vif, skb,
						      peer, action_code,
						      initiator, extra_ies,
						      extra_ies_len);
		break;
	case WLAN_TDLS_SETUP_CONFIRM:
		if (status_code == 0)
			rwnx_tdls_add_setup_cfm_ies(rwnx_hw, rwnx_vif, skb,
						    peer, initiator, extra_ies,
						    extra_ies_len);
		break;
	...
	case WLAN_TDLS_DISCOVERY_REQUEST:
		...
		if (status_code == 0 || action_code == WLAN_TDLS_TEARDOWN)
			rwnx_tdls_add_link_ie(rwnx_hw, rwnx_vif, skb, peer,
					      initiator);
		break;
	}
}
				    ...
				    struct cfg80211_chan_def *chandef)
{
	...
	case WLAN_TDLS_TEARDOWN:
	case WLAN_TDLS_DISCOVERY_REQUEST:
		ret = rwnx_tdls_prepare_encap_data(rwnx_hw, rwnx_vif, peer,
						   action_code, dialog_token,
						   status_code, skb);
		break;

	case WLAN_PUB_ACTION_TDLS_DISCOVER_RES:
		ret = rwnx_prep_tdls_direct(rwnx_hw, rwnx_vif, peer,
					    action_code, dialog_token,
					    status_code, skb);
		break;
	...
	rwnx_tdls_add_ies(rwnx_hw, rwnx_vif, skb, peer, action_code,
			  status_code, initiator, extra_ies, extra_ies_len,
			  oper_class, chandef);
	...
	if (action_code == WLAN_PUB_ACTION_TDLS_DISCOVER_RES) {
		...
		params.len = skb->len;
		params.buf = skb->data;
		ret = rwnx_start_mgmt_xmit(rwnx_vif, NULL, &params, false,
					   &cookie);
#else
		ret = rwnx_start_mgmt_xmit(rwnx_vif, NULL, NULL, false, 0,
					   skb->data, skb->len, false, false,
					   &cookie);
