...
__INLINE void ipc_app2emb_trigger_setf(void *env, u32 app2embtrigger)
{
	...
	REG_IPC_APP_WR(env, IPC_APP2EMB_TRIGGER_INDEX,
		       (u32)app2embtrigger << 0);
}
...
__INLINE void ipc_emb2app_unmask_setf(void *env, u32 emb2appunmask)
{
	...
	REG_IPC_APP_WR(env, IPC_EMB2APP_UNMASK_SET_INDEX,
		       (u32)emb2appunmask << 0);
}
...
{
	ASSERT_ERR((((u32)emb2appunmask << 0) & ~((u32)0xFFFFFFFF)) == 0);
	REG_IPC_APP_WR(env, IPC_EMB2APP_UNMASK_CLEAR_INDEX,
		       (u32)emb2appunmask << 0);
}
