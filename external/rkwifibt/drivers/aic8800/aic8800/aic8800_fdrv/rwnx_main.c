...
u8_l wowlan_param1[] = { 0x01, 0x2a, 0x06, 0xff, 0xff, 0xff, 0xff, 0xff,
...
void rssi_update_txpwrloss(struct rwnx_sta *sta, s8_l rssi)
{
	...
	if (rssi_avg > RSSI_THD_0) {
		if (sta->per_pwrloss != PWR_LOSS_LVL0) {
			if ((jiffies_to_msecs(jiffies - sta->last_jiffies) <
			     PWR_DELAY_TIME) &&
...
void aicwf_pwrloss_worker(struct work_struct *work)
{
	...
	if (!dynamic_pwr) {
		...
		mod_timer(&rwnx_hw->pwrloss_timer,
			  jiffies + msecs_to_jiffies(RSSI_GET_INTERVAL));
		return;
	}
	if (atomic_read(&rwnx_vif->drv_conn_state) ==
	    (int)RWNX_DRV_STATUS_CONNECTED) {
		ret = rwnx_send_get_sta_info_req(rwnx_hw, rwnx_hw->sta_rssi_idx,
						 &cfm);
		if (ret < 0) {
			AICWFDBG(LOGINFO, "rwnx_send_get_sta_info_req: %dn",
				 ret);
			return;
		}
		rssi = (s8)cfm.rssi;
	...
	} else if (atomic_read(&rwnx_vif->drv_conn_state) ==
				  ...
				  struct station_info *sinfo)
{
	...
	case FORMATMOD_HE_ER:
		sinfo->txrate.flags = RATE_INFO_FLAGS_VHT_MCS;
		sinfo->txrate.mcs = ((rate_info->mcsIndexTx & 0xF) > 9 ?
					     9 :
					     (rate_info->mcsIndexTx & 0xF));
		sinfo->txrate.nss = ((rate_info->mcsIndexTx >> 4) & 0x7) + 1;
		break;
#endif
	default:
...
//#endif
	...
	if (wifi_mac_addr != 0) {
		sscanf(wifi_mac_addr, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
