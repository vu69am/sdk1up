...
const char *ftrace_print_mgmt_info(struct trace_seq *p, u16 frame_control,
				   u8 cat, u8 type, u8 p2p)
{
	...
	case (IEEE80211_STYPE_ACTION):
		...
		if (cat == MGMT_ACTION_PUBLIC_CAT && type == 0x9)
			...
			case (P2P_ACTION_GO_NEG_RSP):
				trace_seq_printf(p,
						 ": GO Negociation Response");
				break;
			case (P2P_ACTION_GO_NEG_CFM):
				trace_seq_printf(
					p, ": GO Negociation Confirmation");
				break;
			...
			case (P2P_ACTION_DEV_DISC_REQ):
				trace_seq_printf(
					p, ": Device Discoverability Request");
				break;
			case (P2P_ACTION_DEV_DISC_RSP):
				trace_seq_printf(
					p, ": Device Discoverability Response");
				break;
			case (P2P_ACTION_PROV_DISC_REQ):
				trace_seq_printf(
					p, ": Provision Discovery Request");
				break;
			case (P2P_ACTION_PROV_DISC_RSP):
				trace_seq_printf(
					p, ": Provision Discovery Response");
				break;
			default:
			...
			case 8:
				...
				break;
			case 9:
				trace_seq_printf(p, ":Protected Public %d",
						 type);
				break;
			case 10:
				...
				break;
			case 11:
				trace_seq_printf(p, ":Unprotected WNM %d",
						 type);
				break;
			case 12:
			...
			case 127:
				...
				break;
			default:
				trace_seq_printf(p, ":Unknown category %d",
						 cat);
				break;
			}
		}
...
DECLARE_EVENT_CLASS(
	...
	TP_STRUCT__entry(__field(u16, freq) __field(u8, vif_idx) __field(
		u8, sta_idx) __field(u16, frame_control) __field(u8, action_cat)
				 __field(u8, action_type)
					 __field(u8, action_p2p)),
	TP_fast_assign(
		__entry->freq = freq; __entry->vif_idx = vif_idx;
...
const char *ftrace_print_mu_info(struct trace_seq *p, u8 mu_info)
{
	...
	if (mu_info)
		trace_seq_printf(p, "MU: %d-%d", (mu_info & 0x3f),
				 (mu_info >> 6));
...
TRACE_EVENT(txq_select,
	    ...
	    TP_fast_assign(__entry->txq_idx = txq_idx;
			   ...
			   __entry->skb = skb;),
	    TP_printk("%s pkt_ready_up=%d skb=%p",
		      __print_txq(__entry->txq_idx), __entry->pkt_ready,
		      __entry->skb));
...
DECLARE_EVENT_CLASS(
	txq_reason_template, TP_PROTO(struct rwnx_txq *txq, u16 reason),
	TP_ARGS(txq, reason),
...
TRACE_EVENT(
	push_desc,
	TP_PROTO(struct sk_buff *skb, struct rwnx_sw_txhdr *sw_txhdr,
		 int push_flags),

	TP_ARGS(skb, sw_txhdr, push_flags),

	TP_STRUCT__entry(__field(struct sk_buff *, skb) __field(
		unsigned int, len) __field(u16, tx_queue) __field(u8, hw_queue)
				 __field(u8, push_flag) __field(u32, flag)
					 __field(s8, txq_cred) __field(u8,
								       hwq_cred)
						 __field(u16, pkt_cnt)
							 __field(u8, mu_info)),
...
#ifdef CONFIG_RWNX_FULLMAC
	TP_printk(
		"%s skb=%p (len=%d) hw_queue=%s cred_txq=%d cred_hwq=%d %s flag=%s %s%s%s",
...
#else
	TP_printk(
		"%s skb=%p (len=%d) hw_queue=%s cred_txq=%d cred_hwq=%d %s flag=%x (%s) sn=%d %s",
		__print_txq(__entry->tx_queue), __entry->skb, __entry->len,
		__print_hwq(__entry->hw_queue), __entry->txq_cred,
		__entry->hwq_cred, __print_mu_info(__entry->mu_info),
		__entry->flag,
...
TRACE_EVENT(
	...
	TP_STRUCT__entry(
		__field(struct sk_buff *, skb) __field(u16, txq_idx)
			__field(u8, hw_queue) __array(u8, hw_credit,
						      CONFIG_USER_MAX)
	...
	TP_fast_assign(
		//int i;
		__entry->skb = skb; __entry->txq_idx = txq->idx;
...
DECLARE_EVENT_CLASS(
	ps_template, TP_PROTO(struct rwnx_sta *sta), TP_ARGS(sta),
...
DEFINE_EVENT(ps_template, ps_queue, TP_PROTO(struct rwnx_sta *sta),
	     TP_ARGS(sta));

DEFINE_EVENT(ps_template, ps_push, TP_PROTO(struct rwnx_sta *sta),
	     TP_ARGS(sta));

DEFINE_EVENT(ps_template, ps_enable, TP_PROTO(struct rwnx_sta *sta),
	     TP_ARGS(sta));
...
#ifdef CONFIG_RWNX_FULLMAC
DECLARE_EVENT_CLASS(
	mesh_path_template, TP_PROTO(struct rwnx_mesh_path *mesh_path),
	TP_ARGS(mesh_path),
	...
	TP_fast_assign(
		...
		else __entry->next_hop_sta = 0xff;),
...
TRACE_EVENT(radar_detected,
	    TP_PROTO(u8 chain, u8 region, s16 freq, u8 type, u16 pri),
	    TP_ARGS(chain, region, freq, type, pri),
	    TP_STRUCT__entry(__field(u8, chain) __field(u8, region)
				     __field(s16, freq) __field(u8, type)
					     __field(u16, pri)),
