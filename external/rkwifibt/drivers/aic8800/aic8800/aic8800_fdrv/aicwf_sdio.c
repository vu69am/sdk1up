...
int aicwf_sdio_flow_ctrl(struct aic_sdio_dev *sdiodev)
{
	...
	while (true) {
		ret = aicwf_sdio_readb(sdiodev, SDIOWIFI_FLOW_CTRL_REG,
				       &fc_reg);
		if (ret) {
			return -1;
		}
...
int aicwf_sdio_sleep_allow(struct aic_sdio_dev *sdiodev)
{
	...
	list_for_each_entry_safe(rwnx_vif, tmp, &rwnx_hw->vifs, list) {
		if ((rwnx_hw->avail_idx_map & BIT(rwnx_vif->drv_vif_index)) ==
		    0) {
	...
	if (sdiodev->state == SDIO_ACTIVE_ST && !rwnx_hw->scanning &&
	    !rwnx_hw->is_p2p_alive && !rwnx_hw->is_p2p_connected && dhcped) {
		down(&sdiodev->pwrctl_wakeup_sema);
		if (rwnx_hw->vif_started) {
			sdio_dbg("s\n");
			ret = aicwf_sdio_writeb(sdiodev, SDIOWIFI_SLEEP_REG,
						0x10);
			if (ret)
				sdio_err("Write sleep fail!\n");
		}
					    ...
					    u8 *byte_len)
{
	...
	if (sdiodev->bus_if->state == BUS_DOWN_ST) {
		*byte_len = 0;
		...
		ret = aicwf_sdio_readb(sdiodev, SDIOWIFI_BYTEMODE_LEN_REG,
				       byte_len);
...
int aicwf_sdio_aggr(struct aicwf_tx_priv *tx_priv, struct sk_buff *pkt)
{
	...
	sdio_header[0] = ((pkt->len - sizeof(struct rwnx_txhdr) +
			   sizeof(struct txdesc_api)) &
			  0xff);
	sdio_header[1] = (((pkt->len - sizeof(struct rwnx_txhdr) +
			    sizeof(struct txdesc_api)) >>
			   8) &
			  0x0f);
	...
	while (1) {
		...
		if (!wait_for_completion_interruptible(
			...
			if (sdiodev->bus_if->state == BUS_DOWN_ST)
				break;
			if (sdiodev->state == SDIO_ACTIVE_ST) {
				if ((int)(atomic_read(&sdiodev->tx_priv
							       ->tx_pktcnt) <=
					  0) &&
				    (sdiodev->tx_priv->cmd_txstate == false) &&
				    atomic_read(&sdiodev->rx_priv->rx_cnt) == 0)
					aicwf_sdio_pwr_stctl(sdiodev,
							     SDIO_SLEEP_ST);
				else
...
void aicwf_sdio_hal_irqhandler(struct sdio_func *func)
{
	...
	while (ret || (intstatus & SDIO_OTHER_INTERRUPT)) {
		...
		ret = aicwf_sdio_readb(sdiodev, SDIOWIFI_BLOCK_CNT_REG,
				       &intstatus);
	}
	...
	if (intstatus > 0) {
		if (intstatus < 64) {
			...
			aicwf_sdio_intr_get_len_bytemode(
				sdiodev, &byte_len); //byte_len must<= 128
...
int aicwf_sdio_func_init(struct aic_sdio_dev *sdiodev)
{
	...
	//1: no byte mode
	ret = aicwf_sdio_writeb(sdiodev, SDIOWIFI_BYTEMODE_ENABLE_REG,
				byte_mode_disable);
	if (ret < 0) {
		sdio_err("reg:%d write failed!\n",
