...
const int nx_txdesc_cnt_msk[] = {
	NX_TXDESC_CNT0 - 1, NX_TXDESC_CNT1 - 1,
	NX_TXDESC_CNT2 - 1, NX_TXDESC_CNT3 - 1,
#if NX_TXQ_CNT == 5
	NX_TXDESC_CNT4 - 1,
#endif
};
...
static void ipc_host_rxdesc_handler(struct ipc_host_env_tag *env)
{
	...
	do {
		...
		if (env->cb.recv_data_ind(
			    env->pthis,
			    env->ipc_host_rxbuf_array[env->ipc_host_rxbuf_idx]
				    .hostid) != 0)
...
static void ipc_host_radar_handler(struct ipc_host_env_tag *env)
{
	...
	while (env->cb.recv_radar_ind(
		       env->pthis,
		       env->ipc_host_radarbuf_array[env->ipc_host_radarbuf_idx]
			       .hostid) == 0)
		;
...
{
	while (env->cb.recv_unsup_rx_vec_ind(
		       env->pthis, env->ipc_host_unsuprxvecbuf_array
					   [env->ipc_host_unsuprxvecbuf_idx]
						   .hostid) == 0)
		;
}
...
static void ipc_host_msg_handler(struct ipc_host_env_tag *env)
{
	...
	// filled within the time needed for this irq handling
	// call the external function to indicate that a RX packet is received
	while (env->cb.recv_msg_ind(
		       env->pthis,
		       env->ipc_host_msgbuf_array[env->ipc_host_msge2a_idx]
			       .hostid) == 0)
		;
...
static void ipc_host_msgack_handler(struct ipc_host_env_tag *env)
{
	...
	ASSERT_ERR(
		env->msga2e_cnt ==
		(((struct lmac_msg *)(&env->shared->msg_a2e_buf.msg))->src_id &
		 0xFF));
...
static void ipc_host_dbg_handler(struct ipc_host_env_tag *env)
{
	...
	// filled within the time needed for this irq handling
	// call the external function to indicate that a RX packet is received
	while (env->cb.recv_dbg_ind(
		       env->pthis,
		       env->ipc_host_dbgbuf_array[env->ipc_host_dbg_idx]
			       .hostid) == 0)
		;
				    ...
				    const int queue_idx, const int user_pos)
{
	...
	while (1) {
		...
		void *host_id =
			env->tx_host_id[queue_idx][user_pos][used_idx_mod];
		...
		REG_SW_SET_PROFILING_CHAN(env->pthis,
					  SW_PROF_CHAN_CTXT_CFM_HDL_BIT);
		REG_SW_CLEAR_PROFILING_CHAN(env->pthis,
					    SW_PROF_CHAN_CTXT_CFM_HDL_BIT);
	}
			...
			const int user_pos)
{
	uint32_t used_idx = env->txdesc_used_idx[queue_idx][user_pos];
	void *host_id =
		env->tx_host_id[queue_idx][user_pos]
			       [used_idx & nx_txdesc_cnt_msk[queue_idx]];

	// call the external function to indicate that a TX packet is freed
	if (host_id != 0) {
		// Reset the host id in the array
		env->tx_host_id[queue_idx][user_pos]
			       [used_idx & nx_txdesc_cnt_msk[queue_idx]] = 0;
		   ...
		   struct ipc_shared_env_tag *shared_env_ptr, void *pthis)
{
	...
	env->unsuprxvec_bufsz = max(sizeof(struct rx_vector_desc),
				    (size_t)RADIOTAP_HDR_MAX_LEN) +
				RADIOTAP_HDR_VEND_MAX_LEN +
				UNSUP_RX_VEC_DATA_LEN;
			 ...
			 uint32_t hostbuf)
{
	...
	shared_env_ptr->host_rxdesc[env->ipc_host_rxdesc_idx].dma_addr =
		hostbuf;
	...
	env->ipc_host_rxdesc_idx =
		(env->ipc_host_rxdesc_idx + 1) % IPC_RXDESC_CNT;
			   ...
			   uint32_t hostbuf)
{
	...
	// Save the hostid and the hostbuf in global array
	env->ipc_host_radarbuf_array[env->ipc_host_radarbuf_idx].hostid =
		hostid;
	env->ipc_host_radarbuf_array[env->ipc_host_radarbuf_idx].dma_addr =
		hostbuf;
	...
	// Increment the array index
	env->ipc_host_radarbuf_idx =
		(env->ipc_host_radarbuf_idx + 1) % IPC_RADARBUF_CNT;
				   ...
				   uint32_t hostbuf)
{
	...
	env->ipc_host_unsuprxvecbuf_array[env->ipc_host_unsuprxvecbuf_idx]
		.hostid = hostid;
	env->ipc_host_unsuprxvecbuf_array[env->ipc_host_unsuprxvecbuf_idx]
		.dma_addr = hostbuf;

	// Copy the hostbuf (DMA address) in the ipc shared memory
	shared_env_ptr->unsuprxvecbuf_hostbuf[env->ipc_host_unsuprxvecbuf_idx] =
		hostbuf;

	// Increment the array index
	env->ipc_host_unsuprxvecbuf_idx =
		(env->ipc_host_unsuprxvecbuf_idx + 1) % IPC_UNSUPRXVECBUF_CNT;
			 ...
			 uint32_t hostbuf)
{
	...
	// Copy the hostbuf (DMA address) in the ipc shared memory
	shared_env_ptr->msg_e2a_hostbuf_addr[env->ipc_host_msge2a_idx] =
		hostbuf;

	// Increment the array index
	env->ipc_host_msge2a_idx =
		(env->ipc_host_msge2a_idx + 1) % IPC_MSGE2A_BUF_CNT;
						 ...
						 const int user_pos)
{
	...
	if (free_idx != (used_idx + nx_txdesc_cnt[queue_idx])) {
		...
		txdesc_free = env->txdesc[queue_idx][user_pos] +
			      (free_idx & nx_txdesc_cnt_msk[queue_idx]);
	} else {
		txdesc_free = NULL;
	}
...
{
	uint32_t free_idx = env->txdesc_free_idx[queue_idx][user_pos] &
			    nx_txdesc_cnt_msk[queue_idx];
	volatile struct txdesc_host *txdesc_pushed =
		env->txdesc[queue_idx][user_pos] + free_idx;
	...
	ipc_app2emb_trigger_setf(env->shared,
				 CO_BIT(user_pos + queue_idx * CONFIG_USER_MAX +
...
void ipc_host_irq(struct ipc_host_env_tag *env, uint32_t status)
{
	...
	if (status & IPC_IRQ_E2A_TXCFM) {
		...
		for (i = 0; i < IPC_TXQUEUE_CNT; i++) {
			...
			{
				uint32_t q_bit =
					CO_BIT(j + i * CONFIG_USER_MAX +
					       IPC_IRQ_E2A_TXCFM_POS);
				if (status & q_bit) {
