...
/* Inform the host that we have switch on the indicated off-channel */
				...
				cfg80211_ready_on_channel(
					...
					roc_elem->chan, NL80211_CHAN_HT20,
					roc_elem->duration, GFP_ATOMIC);
						    ...
						    struct ipc_e2a_msg *msg)
{
	...
	struct rwnx_vif *rwnx_vif =
		NULL; // = container_of(roc_elem->wdev, struct rwnx_vif, wdev);
	...
	if (!roc_elem->mgmt_roc && roc_elem->on_chan) {
		...
		cfg80211_remain_on_channel_expired(
			roc_elem->wdev->netdev, (u64)(rwnx_hw->roc_cookie_cnt),
			roc_elem->chan, NL80211_CHAN_HT20, GFP_ATOMIC);
#elif LINUX_VERSION_CODE < KERNEL_VERSION(3, 8, 0)
		cfg80211_remain_on_channel_expired(
			roc_elem->wdev, (u64)(rwnx_hw->roc_cookie_cnt),
			roc_elem->chan, NL80211_CHAN_HT20, GFP_ATOMIC);
#else
		cfg80211_remain_on_channel_expired(
			roc_elem->wdev, (u64)(rwnx_hw->roc_cookie_cnt),
				       ...
				       struct ipc_e2a_msg *msg)
{
	struct mm_apm_staloss_ind *ind =
		(struct mm_apm_staloss_ind *)msg->param;
					...
					struct ipc_e2a_msg *msg)
{
	...
	if (!rwnx_hw->vif_table[sta->vif_idx]) {
		wiphy_err(rwnx_hw->wiphy,
			  "rwnx_hw->vif_table[sta->vif_idx] is null\n");
		return 0;
	}
#endif
	...
	if (chan != NULL) {
		...
		if (rwnx_hw->wext_scan) {
			list_for_each_entry(scan_re_wext,
					    &rwnx_hw->wext_scanre_list,
					    scanu_re_list) {
				if (!memcmp(scan_re_wext->bss->bssid,
					    bss->bssid, ETH_ALEN)) {
					AICWFDBG(
						LOGDEBUG,
			...
			scan_re_wext = (struct scanu_result_wext *)vmalloc(
				sizeof(struct scanu_result_wext));
			scan_re_wext->ind = (struct scanu_result_ind *)vmalloc(
				sizeof(struct scanu_result_ind));
			scan_re_wext->payload =
				(u32_l *)vmalloc(sizeof(u32_l) * ind->length);

			memset(scan_re_wext->ind, 0,
			       sizeof(struct scanu_result_ind));
			memset(scan_re_wext->payload, 0, ind->length);

			memcpy(scan_re_wext->ind, ind,
			       sizeof(struct scanu_result_ind));
			memcpy(scan_re_wext->payload, ind->payload,
			       ind->length);
					 ...
					 struct ipc_e2a_msg *msg)
{
	...
	if (ind->status_code == 0 &&
		...
		AICWFDBG(
			LOGINFO,
			"%s the disconnection has been requested, return it\r\n",
			__func__);
		goto exit;
	}
	...
	if (!ind->roamed) { //not roaming
		...
		if (ind->status_code != 0) {
			AICWFDBG(LOGINFO,
				 "%s roaming fail to notify disconnect \r\n",
				 __func__);
			...
			AICWFDBG(LOGINFO,
				 "%s roaming success to notify roam \r\n",
				 __func__);
...
#endif
					,
					(const u8 *)ind->bssid.array, req_ie,
					ind->assoc_req_ie_len, rsp_ie,
					ind->assoc_rsp_ie_len, GFP_ATOMIC);
					    ...
					    struct ipc_e2a_msg *msg)
{
	...
	if ((int)atomic_read(&rwnx_vif->drv_conn_state) ==
	    (int)RWNX_DRV_STATUS_DISCONNECTED) {
		AICWFDBG(LOGINFO,
			 "%s, is already disconnected, drop disconnect ind",
			 __func__);
		return 0;
	}
							...
							struct ipc_e2a_msg *msg)
{
	...
	if ((ind->vif_idx > NX_VIRT_DEV_MAX) || !rwnx_vif->up ||
		...
		wiphy_err(
			...
			ind->vif_idx, rwnx_vif->up, RWNX_VIF_TYPE(rwnx_vif),
			ret);
	...
	if (!rwnx_vif->user_mpm) {
		...
		if (ind->estab) {
			if (!rwnx_sta->valid) {
				...
				memcpy(rwnx_sta->mac_addr, ind->peer_addr.array,
				       ETH_ALEN);
...
#ifdef CONFIG_DEBUG_FS
				rwnx_dbgfs_unregister_rc_stat(rwnx_hw,
							      rwnx_sta);
#endif
			} else {
				WARN_ON(0);
					       ...
					       struct ipc_e2a_msg *msg)
{
	...
	if (ind->delete) {
		...
		if (found) {
			...
			mesh_path = (struct rwnx_mesh_path *)kmalloc(
				sizeof(struct rwnx_mesh_path), GFP_ATOMIC);
			...
			if (mesh_path) {
				...
				mesh_path->p_nhop_sta =
					&rwnx_hw->sta_table[ind->nhop_sta_idx];
				memcpy(&mesh_path->tgt_mac_addr,
				       &ind->tgt_mac_addr, MAC_ADDR_LEN);
						...
						struct ipc_e2a_msg *msg)
{
	...
	list_for_each_entry(mesh_proxy, &rwnx_vif->ap.proxy_list, list) {
		if (!memcmp(&ind->ext_sta_addr, &mesh_proxy->ext_sta_addr,
			    ETH_ALEN)) {
			found = true;
			break;
		}
	...
	if (ind->delete &&found) {
		...
		if (mesh_proxy) {
			...
			memcpy(&mesh_proxy->ext_sta_addr, &ind->ext_sta_addr,
			       MAC_ADDR_LEN);
			mesh_proxy->local = ind->local;

			if (!ind->local) {
				memcpy(&mesh_proxy->proxy_addr,
				       &ind->proxy_mac_addr, MAC_ADDR_LEN);
			}
