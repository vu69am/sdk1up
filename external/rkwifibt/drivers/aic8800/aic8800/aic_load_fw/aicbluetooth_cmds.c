...
{
	printk(KERN_CRIT
	       "tkn[%d]  flags:%04x  result:%3d  cmd:%4d - reqcfm(%4d)\n",
...
static int cmd_mgr_queue(struct rwnx_cmd_mgr *cmd_mgr, struct rwnx_cmd *cmd)
{
	...
	if (!defer_push) {
		...
		aicwf_set_cmd_tx((void *)(cmd_mgr->usbdev), cmd->a2e_msg,
...
int rwnx_send_dbg_mem_write_req(struct aic_usb_dev *usbdev, u32 mem_addr,
				u32 mem_data)
{
				...
				u32 boot_type)
{
	...
	start_app_req = rwnx_msg_zalloc(DBG_START_APP_REQ, TASK_DBG,
					DRV_TASK_ID,
