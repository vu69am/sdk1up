...
to_vop_csc_mode(enum drm_color_encoding color_encoding,
...
static int rockchip_vop_init_gamma(struct vop *vop, struct display_state *state)
{
	...
	i = dev_read_stringlist_search(crtc_state->dev, "reg-names",
				       "gamma_lut");
	if (i < 0) {
...
static void vop_post_config(struct display_state *state, struct vop *vop)
{
	...
	u16 hsize = hdisplay *
		    (conn_state->overscan.left_margin +
		     conn_state->overscan.right_margin) /
		    200;
	u16 vsize = vdisplay *
		    (conn_state->overscan.top_margin +
		     conn_state->overscan.bottom_margin) /
		    200;
...
static void vop_mcu_bypass_mode_setup(struct display_state *state,
				      struct vop *vop)
{
	/*
...
static int rockchip_vop_init(struct display_state *state)
{
	...
	map = syscon_regmap_lookup_by_phandle(crtc_state->dev,
					      "rockchip,vo0-grf");
	if (!IS_ERR_OR_NULL(map)) {
	...
	printf("VOP:0x%8p update mode to: %dx%d%s%d, type:%s\n", vop->regs,
	       ...
	       rockchip_get_output_if_name(conn_state->output_if,
					   output_type_name));
	...
	ret = gpio_request_by_name(crtc_state->dev, "mcu-rs-gpios", 0,
				   &vop->mcu_rs_gpio, GPIOD_IS_OUT);
	if (ret && ret != -ENOENT)
	...
	dclk_inv =
		(conn_state->bus_flags & DRM_BUS_FLAG_PIXDATA_DRIVE_NEGEDGE) ?
			1 :
			0;
	/* For improving signal quality, dclk need to be inverted by default on
   * rv1106. */
	...
	VOP_CTRL_SET(vop, dclk_ddr,
		     conn_state->output_mode == ROCKCHIP_OUT_MODE_YUV420 ? 1 :
									   0);
   ...
   */
	if (VOP_MAJOR(vop->version) == 2 && VOP_MINOR(vop->version) == 0xd) {
		if (conn_state->bus_format == MEDIA_BUS_FMT_YUV8_1X24)
			VOP_CTRL_SET(vop, dsp_data_swap,
				     DSP_RG_SWAP | DSP_RB_SWAP);
	}
   ...
   */
	...
	post_csc_mode = to_vop_csc_mode(conn_state->color_encoding,
...
static void scl_vop_cal_scl_fac(struct vop *vop, uint32_t src_w, uint32_t src_h,
				...
				uint32_t pixel_format)
{
	...
	if (!vop->win->scl->ext) {
		VOP_SCL_SET(vop, scale_yrgb_x, scl_cal_scale2(src_w, dst_w));
		VOP_SCL_SET(vop, scale_yrgb_y, scl_cal_scale2(src_h, dst_h));
		if (is_yuv) {
	...
	val = scl_vop_cal_scale(yrgb_ver_scl_mode, src_h, dst_h, false,
				vsu_mode, &vskiplines);
	...
	if (is_yuv) {
		...
		val = scl_vop_cal_scale(cbcr_ver_scl_mode, cbcr_src_h, dst_h,
...
static int rockchip_vop_set_plane(struct display_state *state)
{
	...
	if ((vop->version == VOP_VERSION(2, 2) ||
	     vop->version >= VOP_VERSION(2, 0xd)) &&
	...
	if ((vop->version == VOP_VERSION(2, 2) ||
	    ...
	    (mode->flags & DRM_MODE_FLAG_INTERLACE))
		dsp_sty =
			crtc_y / 2 + mode->crtc_vtotal - mode->crtc_vsync_start;
	dsp_st = dsp_sty << 16 | (dsp_stx & 0xffff);
	/*
   * vop full need to treats rgb888 as bgr888 so we reverse the rb swap to
   * workaround
   */
	if (crtc_state->format == ROCKCHIP_FMT_RGB888 &&
	    VOP_MAJOR(vop_data->version) == 3)
	...
	VOP_WIN_SET(vop, interlace_read,
...
static int rockchip_vop_send_mcu_cmd(struct display_state *state, u32 type,
				     u32 value)
{
	struct crtc_state *crtc_state = &state->crtc_state;
	struct connector_state *conn_state = &state->conn_state;
     ...
     */
		if ((type == MCU_SETBYPASS) && !value) {
			vop_mcu_mode_setup(state, vop);
			ret = clk_set_rate(&crtc_state->dclk,
...
static int rockchip_vop_mode_valid(struct display_state *state)
{
	...
	if (vm.hactive < 32 || vm.vactive < 32 ||
	    ...
	    vm.hfront_porch == 0 || vm.hsync_len == 0 || vm.hback_porch == 0 ||
	    vm.vfront_porch == 0 || vm.vsync_len == 0 || vm.vback_porch == 0) {
   ...
   */
	if (mode->flags & DRM_MODE_FLAG_DBLCLK ||
	    (VOP_MAJOR(vop_data->version) == 2 &&
	     VOP_MINOR(vop_data->version) >= 12 &&
	     conn_state->output_if & VOP_OUTPUT_IF_BT656))
		mode->crtc_clock *= 2;

	mode->crtc_clock *=
		rockchip_drm_get_cycles_per_pixel(conn_state->bus_format);
