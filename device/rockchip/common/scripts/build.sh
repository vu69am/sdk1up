...
setup_environments()
{
	...
	export RK_CUSTOM_OVERLAY_DIR="$RK_COMMON_DIR/overlays/rootfs/custom/root"
...
main()
{
	...
	# Link overlay directory
	if [ ! -d "$RK_SDK_DIR/overlay" ]; then
		ln -rsf "$RK_CUSTOM_OVERLAY_DIR" "$RK_SDK_DIR/overlay"
	fi
